...
class ServoController(Node):
    def __init__(self):
        ...
        self.busy = False
        self.reset_timer = None  # TAMBAHKAN INI
    ...
    def drop_callback(self, msg):
        ...
        self.send_servo_command(servo_ch, pwm_drop)
    ...
    def reset_servo(self):
        ...
        self.send_servo_command(servo_ch, pwm_hold)
    ...
    def send_servo_command(self, channel, pwm):
        ...
        future.add_done_callback(lambda f: self.command_callback(f, channel, pwm))
    
    def command_callback(self, future, channel, pwm):
        try:
            response = future.result()
            if response.success:
                self.get_logger().info(f'Servo {channel} → {pwm} PWM: SUCCESS')
            else:
                self.get_logger().error(f'Servo command failed')
